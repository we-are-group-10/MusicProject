...
from .models import SongInfo, Category, Artist
...
def songlist(request, style) :
    items = SongInfo.objects.filter(category=Category.objects.get(Name=style)) # get()中的等號左邊依照 models 下的欄位名稱，等號右邊自己取
